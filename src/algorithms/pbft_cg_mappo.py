...
2. PBFT共识层聚合proposals → consensus_actions
3. consensus_actions执行环境
4. 收集轨迹(包含consensus_info)
5. GAE计算优势
6. PPO clip更新（使用consensus_actions的log_prob）
...
class PBFTCGMAPPO(BaseAlgorithm):
    ...
    def act(
        ...
        # 计算共识动作的对数概率
        # 对于离散动作，使用共识后的动作重新计算log_prob
        action_log_probs_list = []
        for agent_id in range(self.n_agents):
            agent_obs = obs[agent_id].unsqueeze(0)
            # hidden_state 形状: (num_layers=1, batch=n_agents, hidden_size)
            agent_hidden = hidden_state[:, agent_id, :].unsqueeze(0)  # (1, 1, hidden_dim)
            consensus_action = consensus_actions[agent_id]
    ...
    def update(self, batch: Dict[str, torch.Tensor]) -> Dict[str, float]:
        ...
        使用consensus_actions的log_prob进行更新，
