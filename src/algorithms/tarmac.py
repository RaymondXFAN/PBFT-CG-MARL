...
在CommNet基础上增加注意力机制：
- 签名向量：signature_i = sigmoid(W_s * h_i)
- 注意力权重：alpha_{ij} = softmax(signature_j^T * h_i)
- 通信消息：m_i = sum_j alpha_{ij} * h_j
...
class TarMACActor(nn.Module):
    """
    TarMAC的Actor网络（带注意力通信）
    
    在CommNet基础上增加注意力机制，实现有针对性的通信。
    ...
    def forward(
        ...
        # 调用方 (act / get_action) 可能对 hidden_state 做过 unsqueeze(0)，
        # 而 trainer 传入时若已带 batch 维会产生 4D；单步推理时可能为 2D。
        # 这里统一收敛到 3D (B, n_agents, hidden_dim)，保证 torch.bmm 可用。
        ...
        for step in range(self.comm_steps):
            if all_hidden is not None:
                # 计算签名向量：signature_i = sigmoid(W_s * h_i)
                # all_hidden: (batch, n_agents, hidden_dim)
                signatures = torch.sigmoid(
                    self.signature_layers[step](all_hidden)
                )  # (batch, n_agents, hidden_dim)

                # 计算注意力权重
                # alpha_{ij} = softmax(signature_j^T * h_i)
                # h: (batch, hidden_dim), signatures: (batch, n_agents, hidden_dim)
                attention_scores = torch.bmm(
                    signatures, h.unsqueeze(-1)
                ).squeeze(-1)  # (batch, n_agents)
                attention_weights = F.softmax(attention_scores, dim=-1)  # (batch, n_agents)

                # 计算通信消息：m_i = sum_j alpha_{ij} * h_j
                comm_message = torch.bmm(
                    attention_weights.unsqueeze(1),  # (batch, 1, n_agents)
                    all_hidden,  # (batch, n_agents, hidden_dim)
...
class TarMAC(BaseAlgorithm):
    ...
    def _build_networks(self) -> None:
        """构建TarMAC Actor和Critic网络"""
        if self.share_param:
            self.actor = TarMACActor(
